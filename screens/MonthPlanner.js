import { View, Button, Text, Modal, SafeAreaView, ActivityIndicator, FlatList,Image,ImageBackground,StyleSheet,TouchableOpacity} from 'react-native';
...
export default function MonthPlanner({ navigation }) {
  ...
  let loadToDoList = async () => {
    const q = query(collection(db, "todosMonth"), where("userId", "==", auth.currentUser.uid));
  ...
  let checkToDoItem = (item, isChecked) => {
    const toDoRef = doc(db, 'todosMonth', item.id);
    setDoc(toDoRef, { completed: isChecked }, { merge: true });
  };

  let deleteToDo = async (toDoId) => {
    await deleteDoc(doc(db, "todosMonth", toDoId));
  ...
  let addToDo = async (todo) => {
    ...
    const docRef = await addDoc(collection(db, "todosMonth"), toDoToSave);
  ...
  return (
    ...
    <Text style={styles.day}>Month Planner</Text>
