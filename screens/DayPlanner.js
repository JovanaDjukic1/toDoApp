import { View, Button, Text, Modal, SafeAreaView,StyleSheet, ActivityIndicator, FlatList,Image,TouchableOpacity,ImageBackground} from 'react-native';
...
export default function DayPlanner({ navigation }) {
  ...
  let loadToDoList = async () => {
    const q = query(collection(db, "todosDay"), where("userId", "==", auth.currentUser.uid));
  ...
  let deleteToDo = async (toDoId) => {
    await deleteDoc(doc(db, "todosDay", toDoId));
  ...
  let checkToDoItem = (item, isChecked) => {
    const toDoRef = doc(db, 'todosDay', item.id);
  ...
  let renderToDoItem = ({item}) => {
    return (
      ...
      <View style={[styles.rowContainer, styles.rightMargin, styles.leftMargin]}>
        ...
        <InlineTextButton text="Delete" color="rgb(155,155,155)" onPress={() => deleteToDo(item.id)} />
      </View>
    );
  }
  ...
  return (
    ...
    <Image source={require("../assets/notebookDown.jpg")} 
    style={{ height: 150,width:700,marginLeft:-250, marginTop:50}}></Image>
    <Text style={styles.day}>Day Planner</Text>
