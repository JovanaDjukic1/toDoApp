import { View, Button, Text, Modal, SafeAreaView, ActivityIndicator, FlatList,Image,StyleSheet,ImageBackground,TouchableOpacity} from 'react-native';
...
export default function YearPlanner({ navigation }) {
  ...
  let loadToDoList = async () => {
    const q = query(collection(db, "todosYear"), where("userId", "==", auth.currentUser.uid));
  ...
  let checkToDoItem = (item, isChecked) => {
    const toDoRef = doc(db, 'todosYear', item.id);
    setDoc(toDoRef, { completed: isChecked }, { merge: true });
  };

  let deleteToDo = async (toDoId) => {
    await deleteDoc(doc(db, "todosYear", toDoId));
  ...
  let addToDo = async (todo) => {
    ...
    const docRef = await addDoc(collection(db, "todosYear"), toDoToSave);
  ...
  return (
    ...
    <Text style={styles.day}>Year Planner</Text>
